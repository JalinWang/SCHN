...
method_name = "main_mmasia" # original SCHN
# method_name = "main_tomm" # SCHN with a discriminator


def run():
    args = load_config()

    logdir = f"logs/{method_name}-{args.dataset}-{datetime.now().strftime('%y-%m-%d-%H-%M-%S')}"
    ...
    loc = locals()
    exec(f"from {method_name} import train")
    train = loc["train"]
...
def load_config():
    ...
    """
    parser = argparse.ArgumentParser(description='SCHN Pytorch')

    # ***** The following dataset-related arguments are configurated above *****
    parser.add_argument('--dataset', default="cifar-10", type=str,
                        choices=["cifar-10", "nus-wide-tc21"],
                        help='Dataset name.')
